        ...
        <tr>
            ...
            <td class="project-title">
                <a
                    ...
                    @if (($projectCommentsCount = $project->unreadCommentsCount(auth()->id())) > 0 && $project->doUserHaveAccess(auth()->id()))
                        <span class="label label-danger">{{$projectCommentsCount}}</span>
                    @endif
                    {{$project->name}}</a><br /><small>{{$project->client->name}}
                    {{ __('|_ceated') }} : {{\Carbon\Carbon::parse($project->created_at)->format('d.m.Y')}}</small>
                </a>
            </td>
            <td>
                <strong>
            ...
            <td class="text-right" style="min-width: 100px">
                ...
                data-id="{{$project->id}}" data-route="{{route('projects.destroy')}}" title="{{ __('delete') }}">
