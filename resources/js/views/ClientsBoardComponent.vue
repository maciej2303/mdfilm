    ...
    <div class="row">
      <div class="col-6 col-md-2 m-b-sm">
        ...
        <multiselect
          ...
          :taggable="true"
          @tag="addTag($event, projectStatusFilter, projectStatuses)"
          searchable
      ...
      <div class="col-6 col-md-2 m-b-sm">
        ...
        <multiselect
          ...
          @input="filterProjects"
          :taggable="true"
          @tag="addTag($event, clientFilter, clientsInFilter)"
          searchable
          tag-placeholder="Dodaj nowy tag"
        ></multiselect>
      </div>
        ...
        <div class="ibox item">
          <div class="ibox-content">
            ...
            <draggable
              ...
              @change="log($event, client)"
            >
              <li
                v-for="project in client.projects"
                :key="project.id"
                :style="{
                  'border-left': '3px solid' + project.project_status.colour,
                }"
              >
                <a :href="goToRouteShow(project.id)" style="color: black">
                  <div class="m-b-xs">
                    <span
                      class="label"
                      :style="{
                        'background-color': project.project_status.colour,
                      }"
                      style="color: white"
                      >{{ statusesTranslaction[project.project_status.id] }}
                    </span>
                  </div>
                  <i
...
export default {
  ...
  props: {
    statusesTranslaction : { type: Object, default: [] },
    propClients: { type: Array, default: [] },
    routeShow: { defult: "" },
    projectStatuses: { type: Array, defult: [] },
    projects: { type: Array, defult: [] },
    clientsInFilter: { type: Array, defult: [] },
  ...
  mounted() {
    ...
    this.clients = this.propClients;
  },
  methods: {
    clientsOrder(event) {
      let order = this.clients;
      axios
        .post("/api/client-board/clients-change-order", {
          order,
        })
        .then((response) => {})
        .catch((error) => {});
    ...
    prepareClientFilter() {
      ...
      this.sessionFilters.clientFilter.forEach(function(v) {
        var val = self.clientsInFilter.find(option => option.name === v);
    ...
    prepareStatusFilteres() {
      ...
      this.sessionFilters.projectStatusFilter.forEach(function(v) {
        if(v !== null) {
         var val = self.projectStatuses.find(option => option.model_id === v);
    ...
    log: function (evt, client) {
      let order = this.clients.find((obj) => obj.id === client.id).projects;
      this.changeOrder(order);
      document.querySelector(".agile-board").click();
    },
    changeOrder(order) {
      axios
        .post("/api/client-board/change-order", {
